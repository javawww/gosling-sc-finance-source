 ...
 *
 */
public class UEditorUploaderImage {
    // 输出文件地址
    private String                  url          = "";
    // 上传文件名
    ...
    public UEditorUploaderImage(HttpServletRequest request) {
        ...
        tmp.put("IMAGEERROT", "图片服务器内部错误，请联系管理员");
    }
        ...
        try {
            ...
            while (fii.hasNext()) {
                ...
                if (!fis.isFormField()) {
                    ...
                    String result = uploadUtil.uEditorUploaderImage(file, request);
                    if (result != null && !"null".equals(result)) {
                        if (result.contains("/")) {
                            this.url = DomainUrlUtil.getGOSLING_IMAGE_RESOURCES() + result;
                        } else {
                            this.state = result;
                        }
                    } else {
                        this.state = errorInfo.get("IMAGEERROT");
                    }
                    file.delete();
                    break;
                } else {
